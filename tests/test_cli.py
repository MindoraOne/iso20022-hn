...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema"
    ...
    <xs:element name="Document">
        ...
        with open(self.csv_file, "w") as f:
            f.write("id,date,nb_of_txs,initiator_name,payment_id,payment_method\n")
    ...
    def test_cli_schema_validation_failure(self):
        ...
        with patch('pain001.cli.cli.validate_via_xsd', side_effect=Exception("Validation error")):
            result = self.runner.invoke(main, [
                '-t', 'pain.001.001.03',
                '-m', self.xml_template,
                '-s', self.xsd_schema,
        ...
        with patch('os.path.expanduser', return_value=home_xml):
            with patch('pain001.cli.cli.validate_via_xsd', return_value=True):
                with patch('pain001.cli.cli.process_files'):
                    ...
                    assert "does not exist" not in result.output or result.exit_code != 1
